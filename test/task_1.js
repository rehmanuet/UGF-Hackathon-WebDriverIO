    ...
    it(`${testData.name} ${(testData.expect === true) ? 'should' : "shouldn't"} be displayed on ${testData.device} devices`,() =>
      {
        ...
        assert.strictEqual(Utils.report.hackathonReporter(1,
          `${testData.name} ${(testData.expect === true) ? 'is' : "isn't'"} displayed on ${browser.capabilities.browserName} browser`,
          browser.capabilities.browserName, testData.width, testData.height, testData.device,
          testData.dom, isDisplayed, testData.expect), testData.expect)
      });
  });
});